...
dataset_name <- "2050905_mda"
dataset_name <- "2050905_pta"
dataset_name <- "2050905_saliva"
...
KrakenKneePlot(adata, groupby = "phylum", showNumSpecies=showNumSpecies) ############################### why does it not work? NA-value...
...
################################################################################
################## Bin count-based analysis (mock only) ######################## ------------- bin count; to look for plasmids etc
################################################################################


if(!str_detect(bascetRoot,"saliva")){

  # 'Bacillus pacificus (ATCC 10987) CP086328.1.fasta'  'Cereibacter sphaeroides (ATCC 17029) pRSPH01.fasta'  'Deinococcus radiodurans (ATCC 13939) pMP1.fasta'   'Staphylococcus epidermidis (ATCC 12228).fasta'
  # 'Bacillus pacificus (ATCC 10987) CP086329.1.fasta'  'Clostridium beijerinckii (ATCC 35702).fasta'	  'Enterococcus faecalis (ATCC 47077) chr1.fasta'     'Streptococcus mutans (ATCC 700610).fasta'
  # 'Bifidobacterium adolescentis (ATCC 15703).fasta'   'Deinococcus radiodurans (ATCC 13939) chr1.fasta'	  'Enterococcus faecalis (ATCC 47077) plasmid.fasta'
  # 'Cereibacter sphaeroides (ATCC 17029) chr1.fasta'   'Deinococcus radiodurans (ATCC 13939) chr2.fasta'	  'Escherichia coli (ATCC 700926).fasta'
  # 'Cereibacter sphaeroides (ATCC 17029) chr2.fasta'   'Deinococcus radiodurans (ATCC 13939) pCP1.fasta'	  'Lactobacillus gasseri (ATCC 33323).fasta'
  ...
  cnt_all <- ReadBascetCountMatrix(bascetRoot,"bincount", verbose=FALSE)
  
  #cnt_all <- cnt
  
  #colnames(cnt$X)
  cnt_bacillus <- cnt$X[,str_detect(colnames(cnt$X), stringr::fixed("CP086328.1")) | str_detect(colnames(cnt$X), stringr::fixed("CP086329.1"))]
  
  
  adata <- CreateSeuratObject(
    counts = CreateAssayObject(t(cnt_bacillus)), ## according to anndata standard, there should be a transpose here, unless we abstract this step 
  ...
  adata <- ScaleData(adata, features = rownames(adata))
  adata <- RunPCA(adata, features = VariableFeatures(object = adata))
  adata <- RunUMAP(adata, dims = 1:50, reduction.name = "chrom_umap")  
  
  
  adata <- FindNeighbors(adata)#, dims = 1:10)
  adata <- FindClusters(adata, resolution = 0.05)
  
  plot(sort(adata$nCount_chrom_cnt))
  
  adata$is_high <- paste("h",adata$nCount_chrom_cnt>100)  #50, 100, 300
  Idents(adata) <- adata$is_high
  
  adata@assays$chrom_cnt@counts
  
  rownames(adata@assays$chrom_cnt@counts)
  amount_8 <- colSums(adata@assays$chrom_cnt@counts[str_detect(rownames(adata@assays$chrom_cnt@counts), stringr::fixed("CP086328.1")),])
  amount_9 <- colSums(adata@assays$chrom_cnt@counts[str_detect(rownames(adata@assays$chrom_cnt@counts), stringr::fixed("CP086329.1")),])
  
  adata$amount_8 <- amount_8
  adata$amount_9 <- amount_9
  
  #tail(rownames(adata))
  
  DimHeatmap(adata, dims = 1:2, cells = 500, balanced = TRUE)
  
  DimPlot(object = adata, label = TRUE)
  
  DimPlot(adata, reduction = "pca") + NoLegend()
  ...
  adata$nFeature_chrom_cnt
  
  
  #0 1 4
  pbmc.markers <- FindAllMarkers(adata, only.pos = TRUE)
  main_pbmc.markers <- pbmc.markers %>%
    group_by(cluster) %>%
    dplyr::filter(avg_log2FC > 1)
  
  
  main_pbmc.markers
  tail(main_pbmc.markers)
  
  FeaturePlot(adata, features = c(
#    rev(main_pbmc.markers$gene)[1:10]
    main_pbmc.markers$gene[1:10]
  ))
  
  FeaturePlot(adata, features = c(
    main_pbmc.markers[main_pbmc.markers$cluster==1,]$gene
  ))
  ...
  FeaturePlot(adata, features = c(
    "CP086329.1-280001-290001"
#    "CP086328.1-2170001-2180001"
  ))      
  
  
  FeaturePlot(adata, features = c(
    "CP086328.1-2170001-2180001",
    "CP006777.1-800001-810001",
    "CP086329.1-40001-50001",
    "CP150840.1-40001-50001",
    "NC_008618.1-40001-50001",
    
    "NZ-OQ434560.1-40001-50001"
  ))      
